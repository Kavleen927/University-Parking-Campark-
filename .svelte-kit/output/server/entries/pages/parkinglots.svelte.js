import { c as create_ssr_component } from "../../chunks/index-15d7494f.js";
var parkinglots_svelte_svelte_type_style_lang = "";
const css = {
  code: "#title.svelte-t4kskj{text-align:center;font-weight:700}h1.svelte-t4kskj{@apply font-semibold text-xl;}button.svelte-t4kskj{@apply rounded-lg bg-pink-600 text-white w-full py-2 mt-2;}body.svelte-t4kskj{@apply flex flex-col place-items-center place-content-center;}div.svelte-t4kskj{@apply w-full max-w-xs flex flex-col gap-2;}",
  map: null
};
const Parkinglots = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  $$result.css.add(css);
  return `<body class="${"svelte-t4kskj"}"><div class="${"svelte-t4kskj"}"><br><br><br>
        <h1 id="${"title"}" class="${"svelte-t4kskj"}">Parking Lots</h1> 
        <button class="${"svelte-t4kskj"}">7N</button>
        <button class="${"svelte-t4kskj"}">6</button></div>
</body>`;
});
export { Parkinglots as default };
